$src = "C:\Users\USER\.gemini\antigravity\scratch\kamis-pastries\assets"
$pub = "C:\Users\USER\.gemini\antigravity\scratch\kamis-pastries\public\assets"

if (!(Test-Path $src)) { New-Item -ItemType Directory -Path $src -Force }
if (!(Test-Path $pub)) { New-Item -ItemType Directory -Path $pub -Force }
...
foreach ($key in $photos.Keys) {
    ...
    $pubFile = Join-Path $pub $key
    $url = $photos[$key]
    try {
        Invoke-WebRequest -Uri $url -OutFile $srcFile -UserAgent "Mozilla/5.0"
        Copy-Item $srcFile $pubFile -Force
        Write-Host "Downloaded: $key"
    } catch {
        Write-Host "Failed: $key"
    }
}
Write-Host "Done downloading all 25 unique photos!"
