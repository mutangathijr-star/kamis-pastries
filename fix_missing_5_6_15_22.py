...
# GUARANTEED WORKING URLS FOR ALL 25 ITEMS
CAKES = {
    ...
    "15": ("tutti_frutti_cake.jpg", "https://images.unsplash.com/photo-1563729784474-d77dbb933a9e?w=600&auto=format&fit=crop"),
    ...
    "22": ("white_forest_cake.jpg", "https://images.unsplash.com/photo-1588195538326-c5b1e9f80a1b?w=600&auto=format&fit=crop"),
...
for item_id, (fname, url) in CAKES.items():
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if len(data) > 5000:
                with open(s_path, 'wb') as f1, open(p_path, 'wb') as f2:
                    f1.write(data)
                    f2.write(data)
                print(f"ID {item_id.rjust(2)} ({fname.ljust(22)}): DOWNLOADED ({len(data)} bytes) OK")
            else:
                print(f"ID {item_id.rjust(2)} ({fname.ljust(22)}): SMALL FILE ({len(data)} bytes)")
    except Exception as e:
        print(f"ID {item_id.rjust(2)} ({fname.ljust(22)}): ERROR {e}")

print("Verification download complete!")
